...
- `-O output.json` → **перезаписать** файл;
- `-o output.json` → **дописать** новые результаты в существующий файл.

Для данной работы рекомендуется использовать `-O`, чтобы результаты предыдущих запусков не накапливались.
